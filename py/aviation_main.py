...
class aviation:
    def get_scope(self, scope_directory="./data"):
        ...
        Args:
            scope_directory (str, optional): get the data folder directory. Defaults to "./data".
        ...
        try:
            ...
            print("\n\n")
            print(f"Data read complete --- Total {master_data.shape[0]} records across {master_data.shape[1]} columns")
            return master_data
        except:
            raise DataError("Error reading data, BREAKING ISSUE")
        


    def country_scope(self, _data_, country=['USA']): 
        """Scope the dataset based on a country filer, taken in a list argument of the country code that is in dataset.
        Please refer to the ct_iaids dataset for scope defiantion

        Args:
            _data_ (pandas.DataFrame): get the master DataFrame that is used for the scope mapping
            country (list, optional): list arguent for all the countries required for the scope. Defaults to ['USA'].
    ...
    def state_scope(self, _data_, states=[]):
        """Scope the dataset based on the states in a country filer, taken in a list argument of the states 
        in a country code that is in dataset. Please refer to the ct_iaids dataset for scope defiantion

        Args:
            _data_ (pandas.DataFrame): Get the master DataFrame that is used for the scope mapping
            states (list, optional): all the args for state of the country that is in scope. Defaults to [].
    ...
    def remove_dupes(self, _data_, key_column=['ev_id', 'ntsb_no']):
        ...
        Args:
            ...
            key_column (list, optional): primary keys in the dataset. Defaults to ['ev_id', 'ntsb_no'].
    ...
    def total_injuries(self, _data_, ground_scope=['inj_f_grnd', 'inj_m_grnd', 'inj_s_grnd'], flight_scope=['inj_tot_f', 'inj_tot_m', 'inj_tot_s']):
        """Calculate the injuries count for ground, flight and overall. scope is pre-fixed, can be changed on parameters

        Args:
            _data_ (pandas.DataFrame): scoped input for ETL
            ground_scope (list, optional): scope for ground injuries. Defaults to ['inj_f_grnd', 'inj_m_grnd', 'inj_s_grnd'].
            flight_scope (list, optional): scope for flight injuries. Defaults to ['inj_tot_f', 'inj_tot_m', 'inj_tot_s'].

        Returns:
            pandas.DataFrame: The same dataset with 3 additional columns, showing the ground, flight and total injuries
        """
    ...
    # def main(self):
    """Main block to control other functions of the code.
    """
    setup_init()
    master_data = get_scope()
    country_scoped = country_scope(master_data)
    dupes_removed= remove_dupes(country_scoped)
    master = total_injuries(dupes_removed)
